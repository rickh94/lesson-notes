...
<div id={`${itemType}${item.id}-info`} class="my-6 grid grid-cols-5 gap-2 mx-4">
  ...
  {#each item.resources as resource, idx (idx)}
    <label for={`${itemType}${item.id}-resource${idx}`} class="font-bold text-xl col-span-1">Resource</label>
    <input
      bind:value={resource}
      type="text"
      class="rounded mx-4 w-full px-2 py-1 border-2 border-gray-200 focus:border-cool-gray focus:outline-none col-span-4"
  ...
  <div id={`${itemType}${item.id}-add-resource`} class="flex justify-start col-span-5 space-x-2">
    <button on:click={deleteMe} class="px-8 bg-wine py-2 font-bold text-lg rounded text-white shadow shadow-ash-gray">Delete</button>
    <button on:click={addResource} class="px-8 bg-air-superiority-blue py-2 font-bold text-lg rounded text-white shadow shadow-ash-gray">Add Resource</button>
  </div>
</div>
